...
#### Решение
Один пакет накопится за $56 \cdot 2^3 / (128 \cdot 2^{10} = 7 / 2048)$ секунды, после чего за $56 \cdot 2^3 / 2^{20} = 7/16384$ секунды протолкнется на линию, после чего через $0.005$ секунд начнет расшифровываться. Итого $8.85$ миллисекунд.
...
#### Решение
$a = \frac{N}{d} = \frac{N}{d_{delay} + d_{transm}} = \frac{10 пкт}{(0.01 с + (1пкт/(100пкт/c)))} = 500пкт/с$
...
#### Решение
1. Поскольку узкое место - первая линия, на второй линии не будет очереди и  она не повлияет на разницу времен. Задержка распространения по первой линии тоже одинаковая для обоих пакетов. Тогда разница заключается только в задержке передачи по первой линии, равной $\frac{L}{R_S}$.
2. Первый пакет дойдет до маршрутизатора за $\frac{L}{R_S} + d$ и передастся на вторую линию только к моменту $t_{1out} = \frac{L}{R_S} + d + \frac{L}{R_c}$. Второй же пакет дойдет до маршрутизатора дойдет к моменту времени $t_2 = \frac{L}{R_S} + T + \frac{L}{R_S} + d$. Условие, что не будет очереди -- это $t_2 < t_{1out}$, то есть $\frac{L}{R_S} + T + \frac{L}{R_S} + d < \frac{L}{R_S} + d + \frac{L}{R_c}$, то есть $T < \frac{L}{R_C} - \frac{L}{R_S}$
...
#### Решение
1. $\Delta = \frac{L}{R} = \frac{85 \cdot 10^4 бит}{15 \cdot 10^6 бит/c} = 0.0566 с$
2. $\frac{\Delta}{1-\Delta \cdot B} = \frac{0.0566}{1 - 0.0566 \cdot 16} = 0.6$, значит, общая задержка 3.6 секунд.
3. Непопадание в кэш дает нам поправку $B' = B \cdot 0.4$, из чего новая задержка доступа = 0.08с; значит, общее среднее время ответа составляет 3.08 секунд.
