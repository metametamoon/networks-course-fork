...
1. Откройте веб-браузер и перейдите по ссылке gaia.cs.umass.edu/wireshark-labs/alice.txt.
   ...
   на свой компьютер.
2. Перейдите по ссылке: http://gaia.cs.umass.edu/wireshark-labs/TCP-wireshark-file1.html. Сюда вы
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 0
   - 1079771659
   - прибавил один к номеру seq пришедшего пакета
   - по флагу пакета
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   Порядковые номера (относительные): время отправления -> время получения
   441  5.181 -> 5.397 (rtt = 0.216)
   1171 5.182 -> 5.397 (rtt = 0.215)
   2571 5.182 -> 5.404 (rtt = 0.216)
   3971 5.182 -> 5.404 (rtt = 0.222)
   5371 5.182 -> 5.404 (rtt = 0.222)
   6711 5.182 -> 5.404 (rtt = 0.222)
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - 149 КБ / (6.570с - 5.09с) = 100 КБ/с -- разделили объем полезной нагрузки (т.е. размер файла, не учитываем при этом расходники на хедеры) на время передачи файла (т.е. от момента отправления post до подтверждения последней передачи).  
...
Исходный код в папках `ping-app` и `udp-server-lossy`. Потери пакетов детерминированы (теряется в точности каждый пятый) для упрощения тестирования. Код запускается командой `cargo run -- [port]`, где значения порта по умолчанию - 1337 и 1338 соответственно.
