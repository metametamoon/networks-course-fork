...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - ip version, header length, differentiated services field, protocol, source address, destination addreess
      - ip version, header length, differentiated services field, protocol, source address, destination addreess
      - identificatoin, ttl, checksum
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - identification меняется, ttl неизменный
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   (я взял скриншот у Артема Пешкова, т.к. мой вопрос в чате курса все проигнорировали, а у меня на компютере запросы не доходят :( )
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Три фрагмента (целая часть вверх от 3500 / MTU)
   3. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Length, checksum, флаг "more fragments", который не равен 1 только в последнем сегменте
