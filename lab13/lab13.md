...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - transaction id -- это случайное число, выбранное клиентом dhcp протокола для отличия сообщений между именно этим клиентом и dhcp-сервером 
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - ff:ff:ff:ff
   - 
     - Discover: 00.00.00.00 -> ff.ff.ff.ff
     - Offer: 192.168.1.1 -> ff.ff.ff.ff
     - Request: 00.00.00.00 -> ff.ff.ff.ff
     - ACK: 192.168.1.1 -> ff.ff.ff.ff
5. Каков IP-адрес вашего DHCP-сервера?
   - 192.168.1.1
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   ![alt text](image-6.png)
   - срок аренды ограничен, чтобы в случае необновления аренды ip адрес можно было переиспользовать, не затрачивая таким образом ip адреса на мертвых клиентов
   - 1 день
